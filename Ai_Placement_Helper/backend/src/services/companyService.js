...
export async function getCompanyInfo(companyName) {
  try {
    ...
    // If not in database, try to find in our JSON dataset
    console.log(`🔍 Searching JSON dataset for: ${baseCompanyName}`);
    const companyFromDataset = await findCompanyInDataset(baseCompanyName);
    
    if (companyFromDataset) {
      console.log(`✅ Found ${normalizedName} in JSON dataset`);
      
      // Prepare the company data from JSON dataset
      const companyData = {
        name: normalizedName,
        description: companyFromDataset.description || 
        ...
        ...(companyFromDataset.ceo && { ceo: companyFromDataset.ceo }),
        ...(companyFromDataset.revenue && { revenue: companyFromDataset.revenue }),
        source: 'json_dataset'
      };
      ...
      try {
        ...
        console.log(`💾 Saved ${normalizedName} from JSON dataset to database`);
      } catch (saveError) {
    ...
    // If API data is available, store it in the database
    if (apiData && Object.keys(apiData).length > 0) {
      try {
        // Create a new company object
        const newCompany = new Company({
          name: normalizedName,
          description: apiData.description || `Information about ${normalizedName}.`,
          industry: apiData.industry || 'Not specified',
          founded: apiData.founded || 'Not specified',
          headquarters: apiData.headquarters || 'Not specified',
          employeeCount: apiData.employeeCount || 'Not specified',
          website: apiData.website || 'Not specified',
          // Include optional fields if present
          ...(apiData.ceo && { ceo: apiData.ceo }),
          ...(apiData.revenue && { revenue: apiData.revenue })
        });
        
        await newCompany.save();
        console.log(`💾 Saved ${normalizedName} from external API to database (Source: ${apiData.source || 'External API'})`);
      } catch (saveError) {
      ...
      return {
        success: false,
        error: `No information found for ${normalizedName}`,
        data: {
          name: normalizedName,
          description: `Information for ${normalizedName} is currently unavailable.`,
    ...
    console.error(`Error in getCompanyInfo for ${companyName}:`, error);
    return {
      success: false,
      error: `Failed to retrieve company information: ${error.message}`,
      data: {
        name: companyName,
        description: `An error occurred while retrieving information for ${companyName}.`,
...
/**
 * Loads the companies dataset from JSON file instead of CSV
...
async function loadCompaniesDataset() {
  ...
  const jsonFilePath = path.resolve('frontend/src/data/companyData.json');
  
  try {
    // Check if file exists before trying to read it
    if (!fs.existsSync(jsonFilePath)) {
      console.warn(`JSON file not found at path: ${jsonFilePath}`);
      logError(new Error(`JSON file not found: ${jsonFilePath}`), 'Dataset Loading');
      companiesDataset = [];
      return companiesDataset;
    }
    ...
    // Extract company information from the JSON structure
    const results = [];
    
    // Assuming the JSON structure has a companies object with company names as keys
    if (data && data.companies) {
      for (const [companyName, companyData] of Object.entries(data.companies)) {
        const company = {
          name: companyName,
          industry: companyData.industry || '',
          location: companyData.headquarters || companyData.location || '',
          country: companyData.country || '',
          founded: companyData.foundedYear || companyData.founded || '',
    ...
    datasetLoaded = true;
    logSuccess('Companies dataset loaded from JSON', { count: results.length });
    return results;
  } catch (error) {
    logError(error, 'JSON Dataset Loading');
    companiesDataset = [];
    return companiesDataset;
  }
}
...
/**
 * Fetches company information from external APIs with fallbacks
 * @param {string} companyName 
 * @returns {Promise<Object>}
 */
async function fetchFromExternalAPI(companyName) {
  try {
    console.log(`Attempting to fetch external data for ${companyName}`);
    
    // Try SerpAPI first since it's generally most reliable
    let companyData = await fetchSerpApiInfo(companyName);
    if (companyData && Object.keys(companyData).length > 0) {
      console.log(`Successfully retrieved data from SerpAPI for ${companyName}`);
      return {
        ...companyData,
        source: 'SerpAPI'
      };
    ...
    companyData = await fetchWikiDataInfo(companyName);
    if (companyData && Object.keys(companyData).length > 0) {
      console.log(`Successfully retrieved data from WikiData for ${companyName}`);
      return {
        ...companyData,
        source: 'WikiData'
      };
    ...
    companyData = await fetchWikipediaInfo(companyName);
    if (companyData && Object.keys(companyData).length > 0) {
      console.log(`Successfully retrieved data from Wikipedia for ${companyName}`);
      return {
        ...companyData,
        source: 'Wikipedia'
      };
    ...
    companyData = await fetchDuckDuckGoInfo(companyName);
    if (companyData && Object.keys(companyData).length > 0) {
      console.log(`Successfully retrieved data from DuckDuckGo for ${companyName}`);
      return {
        ...companyData,
        source: 'DuckDuckGo'
      };
    ...
    return {
      name: companyName,
      description: `Could not find reliable information for ${companyName}.`,
      industry: 'Not specified',
      founded: 'Not specified',
      headquarters: 'Not specified',
      employeeCount: 'Not specified',
